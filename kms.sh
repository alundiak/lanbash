...
case $1 in
	...
	"svnR") #$2 = 710:720 -> will be shown history of revisions
		cd $KMS_TRUNK;
		if [ -n "$2" ]
		then
		 	svn log -r "$2" -v
	...
	"node") #$2 install | uninstall | update | TBD
		#$3 with_proxy = to use proxy in console to proceed with yum installation
...
# kms vmX (need more testing)
		# Extended function to execute terminal in X11 forwarding mode.
		# In this case you will be able to run X-es - any software which need GUI
		# Examples: nautilus, Firefox, gnome-terminal, etc.
		# Software will be executed locally but with access to remote server file structure
